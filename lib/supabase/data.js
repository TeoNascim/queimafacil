...
export async function prepareFinal(tournamentId) {
  const { data, error } = await createClient().rpc("prepare_final", { p_tournament_id: tournamentId });
  if (error) throw error;
  return data;
}

export async function generateFinal(tournamentId, assignments) {
  const { data, error } = await createClient().rpc("generate_final", { p_tournament_id: tournamentId, p_assignments: assignments });
  if (error) throw error;
  return data;
}
